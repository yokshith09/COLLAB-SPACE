...
export default async function ProfilePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    () =>
      prisma.user.findUnique({
        ...
        include: {
          ...
          applications: { select: { createdAt: true } },
          messagesSent: { select: { createdAt: true } },
        },
      }),
    null
  );
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      <ContributionGraph 
        dates={[
          ...user.projectsOwned.map(p => p.createdAt),
          ...user.teams.map(t => t.joinedAt),
          ...user.applications.map(a => a.createdAt),
          ...user.messagesSent.map(m => m.createdAt)
        ]} 
      />
